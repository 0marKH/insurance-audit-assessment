...
- `submission.csv`: exact six template columns, **374 H4 rows: 364 correct and 10
...
- `outputs/hospital_4/omitted_invoices.csv`: **461 distinct omitted IDs** (466
  header records); multiple reasons can apply. Coverage **374/835 = 44.79%** of
  H4 IDs; **374/3,942 = 9.49%** across all H2–5 target IDs.
...
- [Manual review](docs/hospital_4_manual_review.md): all ten erroneous invoice
...
approved cap assumptions separately identified,
...
billing unit; conflicting duplicates; assumption-dependent cap corrections;
...
## Approved Hospital 4 update and preserved baseline

Prompt [008](prompts/008_hospital_4_approved_policy_update.md) approves inclusive
exclusion endpoints and earliest lexical retention of exact copies, including
multi-unit copies. Conflicting quantities, rates or eligibility remain unresolved.
These are H4 approvals; H1 price-aware retention and all-patient/whole-term volume
scope are not transferred. Wrong units still require individual quantity evidence.
Matching aliases and acceptance thresholds are unchanged.

The [generated before/after report](outputs/hospital_4/policy_change_report.md)
records **four additions: 000018, 000069, 000084 and 000235**, with **zero removals
and no changed amounts or confidence on the 370 existing rows**. Its JSON companion
includes every changed line/invoice result and remaining omissions.

The cap-corrected rows **000165, 000540 and 000554 remain submitted as
assumption-dependent estimates**. Audit `confirmed_violations` distinguish the
known cap violation from `amount_dependent_violations`; `expected_amount_status`
and `amount_assumptions` identify estimated corrections without disguising them
as established delivery. `outputs/hospital_4/submission_evidence.jsonl` preserves
this distinction alongside the unchanged class confidence. No cap-specific penalty
or calibration is invented. Cap estimates also appear in the review log even when
otherwise eligible for submission. The six-column template remains unchanged.

The full v1 H4 code/policy/output/submission/write-up snapshot is stored under
`baselines/hospital_4_v1/`, with 25 hashed files. Every current run verifies it.
Reproduce the old pipeline in an isolated temporary workspace, without modifying
current outputs:

```bash
python3 scripts/check_hospital_4_baseline.py
```

Original assessment files and frozen H1 implementation/evaluation are still
verified against their original manifests. The historical H1 investigation in
`analysis/hospital_1_label_investigation/` is development analysis using already-
exposed labels, not fresh validation; its historical baseline hash checks refer
to the pre-008 state. No new H1 evaluation or label tuning is performed here.
