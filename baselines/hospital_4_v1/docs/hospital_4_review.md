...
- **Exact exclusion endpoints:** strictly inside N days is certain; exactly N is
  possible and left for review. Hospital 1's approved inclusive boundary is not
  automatically transferred.
- **Duplicate correction:** flag both occurrences, retain delivered context once
  for equal quantities, and withhold correction amounts for the whole group.
  Conflicting quantities make delivered context uncertain. There is no H4 authority
  for Hospital 1's price-aware choice of retained occurrence. Single unambiguous
  cap lines use min(quantity, cap), expressly supported by §6.1.
...
Duplicate corrections provisionally selected inside the frozen pass are removed
before evidence leaves the adapter: payable quantity/amount become null, the
retention key and final multiplication/zeroing steps disappear, and financial
violations dependent on those withdrawn amounts are removed. Contractual duplicate
violations remain. These groups are never submitted. Presence does not depend on
payability. Hospital 1 files, hashes and evaluation remain unchanged.

The run produces all 10,560 line records and 840 header records (835 distinct IDs).
Submission is deliberately narrower: 370 rows, 361 correct and 9 erroneous, 44.31%
of H4 IDs. Omitted: 465 distinct IDs / 470 header records; reasons overlap.
H2, H3 and H5 are entirely omitted: total submission coverage is 370/3,942 (9.39%)
of target IDs. No H1 IDs are submitted. H4 accuracy is unknown without labels.
...
this bounded deliverable. Clarify the three contract gaps before expanding
