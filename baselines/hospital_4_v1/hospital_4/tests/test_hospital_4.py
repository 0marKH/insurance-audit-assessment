...
class Hospital4Tests(unittest.TestCase):
    ...
    def test_exclusion_both_directions_endpoint_review_and_directional_target(self):
        for date, expected in [('2024-01-02',0),('2024-01-14',0),('2024-01-01',None),('2024-01-15',None),('2024-01-16','base')]:
    ...
    def test_duplicate_premium_counts_once_but_withholds_correction(self):
        result,_=self.audit([self.line(PREMIUM,6),self.line(PREMIUM,6,n=2)])
        for row in result['lines']:
            ...
            self.assertIn('duplicate_billing',row['violations'])
            self.assertNotIn('line_amount',row['violations'])
            self.assertNotIn('retained_record_key',row)
        self.assertTrue(all(r['requires_review'] for r in result['invoices']))
    ...
    def test_partial_cap_and_duplicate_cap_group_no_allocation(self):
        ...
        result,_=self.audit([self.line(CAP,5),self.line(CAP,5,n=2)])
