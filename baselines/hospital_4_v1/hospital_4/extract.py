...
def extract(path=None):
    ...
    specs = {
        ...
        "11.3": ("duplicate_billing", {"same_service_patient_day":True,"across_invoices":True,"correction":"review"}),
    }
        ...
        for n, cells in entries[2:]:
            ...
            if sec == "3":
                ...
                add("daily_quantity_cap",{"services":[name]}, {"operator":">","threshold":quantity(cells[1])},
                    {"maximum_billable_units":quantity(cells[1]),"excess_payable_cents":0},["6.1","6.2"],day_group,**kw)
            elif sec == "7":
                a,ra,b,rb=cells
                ...
                add("exclusion_window",{"services":[name]}, {"trigger_service":cells[2],"window_days":quantity(cells[1])["value"],"direction":"both","same_patient":True,"endpoint":"unspecified"},
                    {"type":"not_billable","target_service":name},["9.1"],["patient_id"],questions=["Exactly N days: inclusive or exclusive? Endpoint-only evidence is reviewed."],**kw)
            else:
    ...
    return {"schema_version":"h4-1.0","contract":contract,"rules":rules,"policy":policy,
            ...
            "limitations":["dual_unit_service","cumulative_population_and_horizon","exclusion_exact_endpoints","duplicate_correction"]}
