...
def main():
    ...
    expected_invoices={17:(4939975,25000),104:(763938,0),165:(3413175,-42250),
                       323:(1906175,-1000),540:(2195250,-75125),554:(3608010,-142850),
    ...
    endpoint_pairs=[('H4-L00168-14','H4-L00168-07',10),('H4-L00235-06','H4-L00235-03',21),('H4-L00473-08','H4-L00084-14',7)]
    for a,b,days in endpoint_pairs:
        ...
        assert left['exclusion']['applies'] is None and left['expected_payable_cents'] is None
    for key in ('H4-L00002-15','H4-L00018-07','H4-L00693-16','H4-L00051-01','H4-L00053-15'):
    ...
    print(json.dumps({'reviewed_line_calculations':len(expected_lines),'reviewed_erroneous_invoices':len(expected_invoices),
                      'reviewed_endpoint_pairs':len(endpoint_pairs),'other_review_cases':5,
