...
def run():
    ...
    files = {'rules.json':pretty(reference), 'line_audit.jsonl':jsonl(result['lines']),
             ...
             'review_log.jsonl':jsonl([r for r in result['lines'] if r['uncertainty'] or r['expected_payable_cents'] is None])}
    ...
    manifest = {'implementation_sha256':{p.relative_to(ROOT).as_posix():digest(p) for p in sorted(paths)},
                ...
                'labels_used_for_h4_matching_or_pricing':False, 'h1_labels_used_only_for_confidence':True}
